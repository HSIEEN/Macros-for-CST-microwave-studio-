...
Const macrofile = "C:\Program Files\CST Studio Suite 2026\Library\Macros\Coros\Post-Process\Store Resonant Frequency and Q.mcr"
Sub Main
	...
	length_factor = CDbl(dlg.length_factor)
	...
	If feed.getDimensions(deltaX,deltaY,deltaZ)=False Then
		MsgBox "zero element dimensions.", vbCritical
		Exit All
	End If
	x=0
	tgtLength = Round(3e8/(tgtFreq*1e6)/length_factor,1) ' unit mm
	initialTgtLength = tgtLength
	resFreq = 0
	ReportInformationToWindow "*********Routing of antenna starts at " + CStr(Now) +"************"
	Print #fileNumber, "***********Routing of antenna starts at " + CStr(Now) +"**********"
	Dim allIntermediateConLogics As String
	Dim allAntConLogics As String
	Dim ifreq As Integer
	Dim resFreqArray() As Double
	Dim lastResFreq As Double
	Dim lastTgtLength As Double
	...
	Do
		...
		ifSuccess = False
		y=0
		allIntermediateConLogics="$"
		lastResFreq = 0.0
		While ant.constructor(tgtLength, nSolids_CST, deltaX, deltaY, deltaZ) = True
			If InStr(allIntermediateConLogics, "$" & ant.conLogics & "$") <> 0 Or InStr(allAntConLogics, "$" & ant.conLogics & "$") <> 0 Then
				ReportInformationToWindow "%% Antenna logics no way to extend, restart routing..."
				Print #fileNumber, "%% Antenna logics no way to extend, restart routing..."
				Exit While
			End If
			y += 1
			ifSuccess = True
			...
			MacroRun(macrofile)
			resFreqArray = getResonanceFrequency()
			On Error Resume Next ' 开启错误忽略
   				UBound(resFreqArray) ' 尝试获取数组上界，空数组会报错
    			isEmptyArr = (Err.Number <> 0) ' 报错=数组为空
   			On Error GoTo 0
			If isEmptyArr Then
				resFreq = Solver.GetFmax
			Else
				resFreq = getResonanceFrequency()(0)
			End If

			If Abs(resFreq-lastResFreq)<1e-6 Then
				ReportInformationToWindow "%% Resonance frequency "+ CStr(Round(resFreq,2)) + "GHz"+" is same as last one. Restart routing..."
				Print #fileNumber, "%% Resonance frequency "+ CStr(Round(resFreq,2)) + "GHz"+" is same as last one. Restart routing..."
				DeleteResults
				Exit While
			End If
			'Amend the target length when the resonance frequency does not meet the target
			If Abs((resFreq-tgtFreq)/tgtFreq)>=0.04 And Abs((resFreq-tgtFreq)/tgtFreq)<0.5 Then
				...
				'lastTgtLength = tgtLength
				tgtLength = (resFreq/tgtFreq)*ant.length
				lastResFreq = resFreq
				ReportInformationToWindow "%%Intermediate Logics: " & ant.conLogics
				Print #fileNumber, "%%Intermediate Logics: " & ant.conLogics
				allIntermediateConLogics = allIntermediateConLogics & ant.conLogics & "$"
			...
			ElseIf Abs((resFreq-tgtFreq)/tgtFreq)<0.04 Then
				ReportInformationToWindow "%% Simulation done, the resonance frequency is: " + CStr(Round(resFreq,2)) + "GHz"
				Print #fileNumber, "%% Simulation done, the resonance frequency is: " + CStr(Round(resFreq,2)) + "GHz"
				...
				Else
					...
					allAntConLogics = allAntConLogics & ant.conLogics & "$"
					...
					ReportInformationToWindow "%%Radiation efficiency @target frequency: " & CStr(Round(radEffi, 2))&"dB"
					Print #fileNumber, "%% Radiation efficiency @target frequency: " & CStr(Round(radEffi, 2))&"dB"
					ReportInformationToWindow "%%Total efficiency @target frequency: " & CStr(Round(totEffi, 2))&"dB"
					Print #fileNumber, "%% Total efficiency @target frequency: " & CStr(Round(totEffi, 2))&"dB"
			...
			ElseIf Abs((resFreq-tgtFreq)/tgtFreq)>=0.5 Then
				ReportInformationToWindow "%% Simulation done, the resonance frequency is parsed wrong or too far from target frequency. Restart routing!"
				Print #fileNumber, "%% Simulation done, the resonance frequency is parsed wrong or too far from target frequency. Restart routing!"
				DeleteResults
				tgtLength = initialTgtLength
				Exit While
			'Rebuild
			End If
			Plot.Update
...
Sub antennaDesign_initialize(cOrR As Boolean)
	...
	Dim Vertices(0 To 7, 0 To 2) As Double
	If (nSolids_CST > 0) Then
		For iSolid_CST = 1 To nSolids_CST
			...
			With antElem_arr(iSolid_CST-1)
				...
				End If
				For i=0 To 7 STEP 1
					Solid.getPointCoordinates(sFullSolidName,CStr(i+1),x,y,z)
					'Pick.PickPointFromCoordinates(x,y,z)
					Vertices(i, 0) = x
					Vertices(i, 1) = y
					Vertices(i, 2) = z
				Next
